      ...
      <form class="auth-form" @submit.prevent="handleRegister">
        ...
        <div class="form-item">
          ...
          <input
            ...
            class="form-input"
          />
          <div class="error-tip" v-if="form.confirmPwd && form.password !== form.confirmPwd">
            两次密码输入不一致
          </div>
        </div>
...
const isConfirmPwdValid = computed(() => form.value.password === form.value.confirmPwd);
...
const handleRegister = async () => {
  try {
    ...
    alert('注册失败：' + (err.response?.data?.msg || '注册失败'));
  }
};
